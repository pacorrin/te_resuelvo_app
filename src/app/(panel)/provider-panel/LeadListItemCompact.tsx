...
import { Eye, EyeOff, Wrench, Check, Link as LinkIcon } from "lucide-react";
...
export function LeadListItemCompact({ lead, organizationId }: { lead: LeadListItemData, organizationId: number }) {
  ...
  const handlePurchase = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    let result = await _initPurchaseProcess({
      tenderId: lead.id,
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 flex items-end justify-between gap-2">
        ...
        {lead.status === "available" && (
          <Button onClick={handlePurchase} className="inline-flex h-7 items-center rounded-lg bg-blue-600 px-3 text-[10px] font-semibold text-white shadow-sm">
            Adquirir
          </Button>
        )}
      </div>
